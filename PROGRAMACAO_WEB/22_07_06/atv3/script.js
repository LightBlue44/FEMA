...
function exibirItens() {
    ...
    for (let i = 0; i < itens.length; i++) {
        if(itens[i].assistido) {
            ...
            template = template + `<button type="button" class="material-icons md-light" onclick="Assistido('${itens[i].id}')">visibility</button><div id="Assistido">Assistido</div>`;
            ...
            template = template + `<button type="button" class="material-icons md-light" onclick="editarItem('${itens[i].id}')">edit</button>`;
            template = template + `<button type="button" class="material-icons md-light" onclick="excluirItem('${itens[i].id}')">delete</button>`;
            template = template + `<button type="button" class="material-icons md-light" onclick="Assistido('${itens[i].id}')">visibility_off</button><div id="Assistido">Não Assistido</div>`;
            template = template + '</div>';
            template = template + '</div>';
        }
    }
...
function excluirItem(id) {
    let r = confirm("Certeza que deseja excluir o vídeo?")
    if (r == true) {
        console.log('id ', id);
        let index = itens.findIndex(f => f.id == id);
